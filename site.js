(function () {
  ...
  // only dated story; the sustainability story remains unpublished until a real
  ...
  const newsItems = [
    ...
    { id: "singapore-client-factory-visit-2026", category: "company", label: "COMPANY UPDATE", date: "2026-08-04", displayDate: "04 AUG 2026", title: "Overseas Client Factory Visit", excerpt: "Welcoming our overseas client to WINKO for a factory tour, technical discussion and closer look at our water tank manufacturing capabilities.", image: "index_html_files/singapore-factory-visit-2026.png", alt: "WINKO factory visit with an overseas client reviewing water tank manufacturing facilities", href: "news/singapore-client-factory-visit-2026.html", cta: "READ STORY", featured: true },
  ...
  function updateStructuredData(locale, page, pageUrl, detailTarget) {
    ...
    const article = page === "news-article" ? newsItems.find((item) => item.id === (articleTarget?.dataset.newsArticleId || "singapore-client-factory-visit-2026")) : null;
    const articleLocale = article ? translate(article.articleHeading || (article.id === "singapore-client-factory-visit-2026" ? "Overseas Client Visits WINKO Factory" : article.title), locale) : "";
    if (article) {
  ...
  function updateMetadata(locale) {
    ...
    const values = detailKey && I18N.productMeta?.[detailKey]?.[locale] ? I18N.productMeta[detailKey][locale] : (I18N.meta?.[metaPage]?.[locale] || I18N.meta?.[metaPage]?.en || I18N.meta?.[page]?.[locale] || I18N.meta?.[page]?.en);
    ...
    const articleData = newsItems.find((item) => item.id === (newsArticleId || "singapore-client-factory-visit-2026"));
  ...
  function setupNewsArticle() {
    ...
    if (!target) return;
    const articleId = target.dataset.newsArticleId || "singapore-client-factory-visit-2026";
    ...
    const render = () => {
      const locale = currentLanguage();
      target.innerHTML = `<article class="news-article" aria-labelledby="news-article-title"><section class="news-article-hero"><div class="container news-article-intro" data-reveal><p class="eyebrow">${translate("NEWS", locale)}</p><span class="article-meta">${translate("04 AUGUST 2026", locale)}</span><h1 id="news-article-title">${translate("Overseas Client Visits WINKO Factory", locale)}</h1></div><div class="container"><figure class="news-article-media${article.id === "responsible-manufacturing-sustainability-2026" ? " news-article-media--poster" : ""} news-media--blurred" data-reveal><img src="${article.image}" ${imageAttributes(article.image)} loading="eager" fetchpriority="high" decoding="async" alt="${newsText(article, "alt", locale)}"></figure></div></section><section class="section news-article-content"><div class="container"><div class="news-article-body" data-reveal>${paragraphs.map((paragraph) => `<p>${translate(paragraph, locale)}</p>`).join("")}<p class="news-article-signoff">${translate("WINKO - Engineered Water Storage Solutions", locale)}</p><a class="text-link news-text-link news-back-link" href="news.html">${translate("BACK TO NEWS", locale)}${arrowIcon}</a></div></div></section></article>`;
      setupReveal();
      const titleNode = target.querySelector("#news-article-title");
      const metaNode = target.querySelector(".news-article-intro .article-meta");
      const mediaNode = target.querySelector(".news-article-media");
      const signoffNode = target.querySelector(".news-article-signoff");
      const articleHeading = article.articleHeading || (article.id === "singapore-client-factory-visit-2026" ? "Overseas Client Visits WINKO Factory" : article.title);
      if (titleNode) titleNode.textContent = translate(articleHeading, locale);
      if (metaNode) metaNode.textContent = `${newsText(article, "label", locale)} · ${article.displayDate ? translate(article.displayDate, locale) : translate("UNPUBLISHED", locale)}`;
      if (mediaNode && article.id !== "singapore-client-factory-visit-2026") mediaNode.classList.remove("news-media--blurred");
